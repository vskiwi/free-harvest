 ...
 * ("0,Celsius, "), which the file client reads with one FILEREAD (docs/30 §5).
 * hr_dryerfiles does that read after every link-up and on request, and hands
 * the answer here (hr_units_set_dryer_unit); it is kept in RAM and NVS so a
 * reboot starts from the last known panel unit. AUTO then resolves to it
 ...
 * sync is a read, and lives in hr_dryerfiles).
 */
#ifndef HR_UNITS_H
#define HR_UNITS_H
